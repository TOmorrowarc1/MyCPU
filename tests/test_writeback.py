...
# 导入通用测试工具 (假设你已经按之前的建议创建了 tests/common.py)
...
class WBDriver(Module):
    ...
    def build(self, dut: Module):
        ...
        log("[Driver Heartbeat] Cycle: {}", idx)
        ...
        # 5. 结束仿真
        # 跑完所有测试后多跑几拍，确保 Log 打印完成
        with Condition(idx > UInt(32)(len(vectors) + 2)):
            finish()
...
def check(raw_output):
    ...
    if len(captured_writes) != len(expected_writes):
        print(f"❌ 错误：预期写入 {len(expected_writes)} 次，实际写入 {len(captured_writes)} 次")
...
if __name__ == "__main__":
    ...
    # 2. 运行测试 (使用 tests/common.py 中的工具)
