...
def check(raw_output):
    ...
    expected_sram_ops = [
        ("EX_STORE", 0x1000, 0x12345678),  # Case 0: SW (EX阶段输出地址=0x1000, MEM阶段实际存储数据=0x12345678)
        ("EX_STORE", 0x1004, 0xABCDEF00),  # Case 1: SW (EX阶段输出地址=0x1004, MEM阶段实际存储数据=0xABCDEF00)
        ("EX_STORE", 0x1008, 0x11223344),  # Case 2: SW (EX阶段输出地址=0x1008, MEM阶段实际存储数据=0x11223344)
        ("EX_LOAD", 0x1000, 0x12345678),  # Case 3: LW (EX阶段输出地址=0x1000, MEM阶段实际加载数据=0x12345678)
        ("EX_LOAD", 0x1004, 0xABCDEF00),  # Case 4: LW (EX阶段输出地址=0x1004, MEM阶段实际加载数据=0xABCDEF00)
        ("EX_LOAD", 0x1008, 0x11223344),  # Case 5: LW (EX阶段输出地址=0x1008, MEM阶段实际加载数据=0x11223344)
        ("EX_STORE", 0x1001, 0x55555555),  # Case 6: SW (EX阶段输出未对齐地址=0x1001, MEM阶段尝试存储数据=0x55555555)
        ("EX_LOAD", 0x1003, None),  # Case 7: LW (EX阶段输出未对齐地址=0x1003, MEM阶段尝试加载数据)
        ("EX_STORE", 0x1010, 0xABCD),  # Case 8: SH (EX阶段输出地址=0x1010, MEM阶段实际存储数据=0xABCD)
        ("EX_LOAD", 0x1010, 0xABCD),  # Case 9: LH (EX阶段输出地址=0x1010, MEM阶段实际加载数据=0xABCD)
        ("EX_STORE", 0x1011, 0xEF),  # Case 10: SB (EX阶段输出地址=0x1011, MEM阶段实际存储数据=0xEF)
        ("EX_LOAD", 0x1011, 0xFFFFFFEF),  # Case 11: LB (EX阶段输出地址=0x1011, MEM阶段实际加载数据=0xFFFFFFEF)
        ("EX_STORE", 0x1020, 0x12345678),  # Case 12: SW (EX阶段输出地址=0x1020, MEM阶段实际存储数据=0x12345678)
        ("EX_LOAD", 0x1020, 0x12345678),  # Case 13: LW (EX阶段输出地址=0x1020, MEM阶段实际加载数据=0x12345678)
        ("EX_STORE", 0x1021, 0x1234),  # Case 14: SH (EX阶段输出未对齐地址=0x1021, MEM阶段尝试存储数据=0x1234)
        ("EX_LOAD", 0x1023, None),  # Case 15: LH (EX阶段输出未对齐地址=0x1023, MEM阶段尝试加载数据)
    ]
    ...
    # 4. SRAM操作内容检查
    sram_op_idx = 0
    for i, sram_op in enumerate(expected_sram_filtered):
